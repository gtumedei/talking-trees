...
export function buildContextString(treeStructure: TreeStructure | null): string {
  ...
  if (s["DATI_ALBERO"]) {
    ...
    parts.push(
      "Gli alberi monumentali rappresentano un patrimonio naturale e culturale inestimabile e la loro conservazione è importante per l'ambiente e il territorio.",
    )
  }
  ...
  if (s["DATI_SPECIE_BOTANICHE"]) {
    ...
    if (sp.info_forma_chioma || sp.chioma?.forma)
      caratteristiche.push(String(sp.info_forma_chioma || sp.chioma?.forma).toLowerCase())
    if (sp.info_colori_autunnali || sp.colori_autunnali)
      caratteristiche.push(`colori autunnali: ${sp.info_colori_autunnali || sp.colori_autunnali}`)
    if (sp.info_frutti || sp.frutti) caratteristiche.push(`frutti: ${sp.info_frutti || sp.frutti}`)
    if (sp.info_fioritura || sp.fioritura)
      caratteristiche.push(`fioritura: ${sp.info_fioritura || sp.fioritura}`)
    ...
    if (
      sp.size_altezza ||
      sp.dimensioni_specie?.altezza_m ||
      sp.size_chioma ||
      sp.dimensioni_specie?.chioma
    )
      specie.push(
        `- Dimensioni (specie):${
          sp.size_altezza
            ? ` Altezza: ${sp.size_altezza}`
            : sp.dimensioni_specie?.altezza_m
              ? ` Altezza: ${sp.dimensioni_specie.altezza_m}`
              : ""
  ...
  if (s["DATI_ECOLOGICI/AMBIENTALI"]) {
    ...
    const eco = Object.entries(ecoData).filter(
      ([_, obj]) => obj?.valore && String(obj.valore).trim(),
    )
    parts.push(
      "Gli alberi svolgono un ruolo fondamentale nel mantenimento dell’equilibrio ambientale: assorbono anidride carbonica, migliorano la qualità dell’aria, favoriscono la biodiversità e contribuiscono alla regolazione del clima locale. La loro presenza aiuta a ridurre il rischio di erosione del suolo, stabilizza i terreni e crea microhabitat preziosi per numerose specie animali.",
    )
    if (eco.length) {
  ...
  if (s["DATI_SALUTE"]) {
    ...
    const salute: string[] = []
    if (h.stato_salute && h.stato_salute !== "Non specificato")
      salute.push(`- Stato di salute: ${h.stato_salute}`)
  ...
  parts.push("\nDATI GENERALI:")
  parts.push(
    "Un albero non è solo un organismo isolato, ma un piccolo ecosistema: offre riparo, cibo e siti riproduttivi a insetti, uccelli, piccoli mammiferi e microorganismi. La sua chioma, la corteccia e il terreno circostante ospitano una rete complessa di vita che contribuisce alla resilienza ambientale dell’area.",
  )
  parts.push(
    "La presenza degli alberi favorisce il benessere psicofisico: migliorano la qualità dell’aria, riducono il rumore e creano zone d’ombra che abbassano le temperature estive. Il contatto con la natura è associato a benefici emotivi, cognitivi e sociali, rendendo questi giganti verdi preziosi anche per la salute dell’uomo.",
  )
  parts.push(
    "Ogni albero monumentale è un’eredità naturale che ci è stata affidata. La sua tutela garantisce che le generazioni future possano continuare ad ammirarlo e a beneficiare dei servizi ecosistemici e del valore culturale che esso rappresenta.",
  )
...
export function buildEcologicalSection(
  species?: UserSpeciesType,
  pollutionData?: CSVRow[] | null,
): Section<EcologicalData> | null {
  if (!species) return null
  ...
  const pollutantInfo: Record<string, { descrizione: string }> = {
    "CO₂": {
      descrizione:
        "La CO₂ è l'anidride carbonica, un gas serra che contribuisce al riscaldamento globale.",
    },
    PM10: {
      descrizione:
        "Il PM10 è un insieme di particelle sottili sospese nell'aria, dannose per l'apparato respiratorio.",
    },
    "O₃": {
      descrizione:
        "L'O₃ (ozono troposferico) è un inquinante secondario che si forma in presenza di sole e smog.",
    },
    "NO₂": {
      descrizione:
        "Il NO₂ (biossido di azoto) deriva dai gas di scarico e influisce sulla salute dei polmoni.",
    },
    "SO₂": {
      descrizione:
        "Il SO₂ (biossido di zolfo) è prodotto da combustioni industriali e può causare piogge acide.",
    },
  }
  ...
  for (const [nome, valore] of Object.entries(inquinanti)) {
    ...
    ecologicalParts[nome] = {
      ...
      descrizione: pollutantInfo[nome]
        ? { descrizione: pollutantInfo[nome].descrizione }
        : undefined,
    }
  }
...
/** DATI LUOGO */
export function buildDatiLuogo(
  tree: UserTreeType,
  locationData?: CSVRow[] | null,
): Section<LocationData> | null {
...
export function buildHealthSection(
  ...
  if (
    (!content.stato || content.stato === "") &&
    (!content.condizioni_meteo || content.condizioni_meteo === "")
  )
    return null
...
/** DATI STORICI */
export function buildHistoricalSection(
  tree: UserTreeType,
  historicalData?: CSVRow[] | null,
): Section<HistoricalData> | null {
...
export async function loadAsyncData(tree: UserTreeType, species?: UserSpeciesType) {
  ...
  const pollutionPromise =
    species && hasPollutionData(species)
      ? loadPollutionData()
...
export async function buildTreeContext(
  tree: UserTreeType,
  species: UserSpeciesType | null = null,
  variant = "statico",
  setChatbotIsReady?: (value: boolean) => void,
) {
  const { pollutionData, placeData, historicalData, weatherData } = await loadAsyncData(
    tree,
    species || undefined,
  )
  ...
  const ragStructure = await buildTreeStructure(
    tree,
    species || undefined,
    pollutionData,
    placeData,
  ...
  try {
    ...
    // eslint-disable-next-line no-console
    console.log("✅ Spazio RAG inizializzato:", {
      space_id,
      instance_id,
