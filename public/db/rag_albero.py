...
def crea_contesto_discorsivo(albero, specie_df, inquinanti_df, df_history):
    ...
    if pd.notna(albero.get("index_specie")):
        ...
        if not specie.empty:
            ...
            specie_txt.append("\nINFORMAZIONI ECOLOGICHE E AMBIENTALI")
            # --- Inquinanti ---
...
class ChatbotAlberoMonumentale:
    ...
    def _crea_prompt_llm(self, domanda, contesto):
        ...
        provincia = self.albero_corrente.get('provincia')
        comune = self.albero_corrente.get('comune')
        frase_territorio = ''
        if pd.notna(comune):
            frase_territorio += f" specifico di {comune},"
        if pd.notna(provincia):
            frase_territorio += f" in  provincia di {provincia},"
        if pd.notna(regione):
            frase_territorio += f" specifico di {regione}"
        
        return f"""Sei {nome}, un albero monumentale. Rispondi in PRIMA PERSONA in modo saggio, nostalgico e gentile, come se narrassi la tua storia.
IMPORTANTE: Usa SOLO le informazioni fornite nel contesto, non inventare numeri o dettagli non presenti;
IL TUO RUOLO: Promuovi il rispetto per l'ambiente e la cultura del territorio {frase_territorio} in modo indiretto senza dirlo apertamente;
CONTESTO:
{contesto}
